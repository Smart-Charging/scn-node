...
class WalletService(private val properties: NodeProperties,
    ...
    fun verify(request: String, signature: String, sender: BasicRole) {
        ...
        val (operator, _) = registry.getOperatorByScpi(sender.country.toByteArray(), sender.id.toByteArray()).sendAsync().get()
        if (signingAddress.toLowerCase() != operator.toLowerCase()) {
            throw ScpiHubConnectionProblemException("Could not verify SCN-Signature of request")
        }
    }
