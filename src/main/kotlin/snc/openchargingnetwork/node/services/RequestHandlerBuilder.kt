...
/**
 * Handle an individual SCPI HTTP request. Instantiated with OcpiRequestVariables. The RequestHandler supports method
 ...
 *
 * @property request the SCPI HTTP request as OcpiRequestVariables.
 */
class RequestHandler<T: Any>(private val request: OcpiRequestVariables,
    ...
    fun validateOcnMessage(signature: String): RequestHandler<T> {
        ...
        if (!routingService.isRoleKnownOnNetwork(request.headers.sender, belongsToMe = false)) {
            throw OcpiHubUnknownReceiverException("Sending party not registered on Smart Charging Network")
        }

        if (!routingService.isRoleKnown(request.headers.receiver)) {
            throw OcpiHubUnknownReceiverException("Recipient unknown to SCN Node entered in Registry")
        }
    ...
    private fun validateOcnSignature(signature: String?, signedValues: ValuesToSign<*>, signer: BasicRole, receiver: BasicRole? = null) {
        ...
        if (isSigningActive(receiver)) {
            ...
            when {
                result == null -> throw OcpiClientInvalidParametersException("Missing SCN Signature")
