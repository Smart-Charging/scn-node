...
import snc.smartchargingnetwork.node.Application
...
fun setUpNode(registryAddress: String, credentials: Credentials, port: Int, signatures: Boolean = true) {
    ...
    SpringApplicationBuilder(Application::class.java)
            ...
            .run("--server.port=$port",
                    "--scn.node.url=$domain",
                    "--scn.node.privatekey=${credentials.ecKeyPair.privateKey.toString(16)}",
                    "--scn.node.web3.provider=$provider",
                    "--scn.node.web3.contracts.registry=$registryAddress",
                    "--scn.node.signatures=$signatures")
...
fun SignableHeaders.toMap(tokenC: String, signature: String): Map<String, String> {
    ...
    fromCountryCode?.let { map["SCPI-From-Country-Code"] = it }
    fromPartyId?.let { map["SCPI-From-Party-Id"] = it }
    toCountryCode?.let { map["SCPI-To-Country-Code"] = it }
    toPartyId?.let { map["SCPI-To-Party-Id"] = it }
    return map
}
