...
class RoutingServiceTest {
    ...
    private val scnRulesListRepo: ScnRulesListRepository = mockk()
    ...
    fun `prepareLocalPlatformRequest without proxy`() {
        val request = ScpiRequestVariables(
        ...
        every { routingService.getPlatformEndpoint(
                ...
                url = "https://scpi.cpo.com/2.2/tokens")
        ...
        assertThat(url).isEqualTo("https://scpi.cpo.com/2.2/tokens/DE/SNC/abc123")
    ...
    fun `prepareRemotePlatformRequest without proxy`() {
        ...
        every { registry.getOperatorByScpi(
                request.headers.receiver.country.toByteArray(),
                request.headers.receiver.id.toByteArray()).sendAsync().get() } returns Tuple2("", "https://scn.node.net")
        ...
        assertThat(url).isEqualTo("https://scn.node.net")
        assertThat(headers.requestID.length).isEqualTo(36)
    ...
    fun `prepareRemotePlatformRequest with proxy`() {
        ...
        val modifiedRequest = request.copy(
                ...
                proxyResource = "https://actual.cpo.com/scpi/sender/2.2/sessions?limit=10&offset=50; rel =\"next\"")

        every { registry.getOperatorByScpi(
                request.headers.receiver.country.toByteArray(),
                request.headers.receiver.id.toByteArray()).sendAsync().get() } returns Tuple2("", "https://scn-node.provider.net")

        every { routingService.getProxyResource("45", request.headers.sender, request.headers.receiver) } returns
                "https://actual.cpo.com/scpi/sender/2.2/sessions?limit=10&offset=50; rel =\"next\""
        ...
        assertThat(url).isEqualTo("https://scn-node.provider.net")
        assertThat(headers.requestID.length).isEqualTo(36)
        assertThat(headers.signature).isEqualTo(sig)
        assertThat(body).isEqualTo(jsonString)
    }
    ...
    fun `prepareRemotePlatformRequest with altered body`() {
        ...
        val expectedAlteredBody = request.copy(
                headers = request.headers.copy(authorization = ""),
                body = body.copy(responseURL = "https://scn-node.provider.net/commands/CANCEL_RESERVATION/128"))
        ...
        assertThat(signedBody.contains("\"response_url\":\"https://scn-node.provider.net/commands/CANCEL_RESERVATION")).isEqualTo(true)
    ...
    fun getProxyResource() {
        ...
        val resource = "https://some.co/scpi/tokens?limit=10; rel=\"next\""
    ...
    fun `isRoleKnownOnNetwork with belongsToMe flag returns true`() {
        ...
        every { registry.getOperatorByScpi(role.country.toByteArray(), role.id.toByteArray()).sendAsync().get() } returns Tuple2(serverEthAddress, serverURL)
    ...
    fun `isRoleKnownOnNetwork without belongsToMe flag returns true`() {
        val role = BasicRole("XYZ", "CH")
        val serverURL = "https://my.server.com"
        every { registry.getOperatorByScpi(role.country.toByteArray(), role.id.toByteArray()).sendAsync().get() } returns Tuple2("", serverURL)
    ...
    fun getRemoteNodeURL() {
        ...
        every { registry.getOperatorByScpi(role.country.toByteArray(), role.id.toByteArray()).sendAsync().get() } returns Tuple2("", "https://some.node.com")
    ...
    fun `validateReceiver should return REMOTE`() {
        ...
        every { registry.getOperatorByScpi(role.country.toByteArray(), role.id.toByteArray()).sendAsync().get() } returns Tuple2("", "http://localhost:8080")
