...
class CredentialsControllerTest(@Autowired val mockMvc: MockMvc) {
    ...
    @MockkBean
    lateinit var scnRulesListRepo: ScnRulesListRepository
    ...
    fun `When GET credentials then return broker credentials`() {
        ...
        mockMvc.perform(get("/scpi/2.2/credentials")
                ...
                .andExpect(jsonPath("\$.data.url").value("http://localhost:8001/scpi/versions"))
                .andExpect(jsonPath("\$.data.roles", Matchers.hasSize<Array<CredentialsRole>>(1)))
                .andExpect(jsonPath("\$.data.roles[0].role").value("HUB"))
                .andExpect(jsonPath("\$.data.roles[0].party_id").value("SCN"))
                .andExpect(jsonPath("\$.data.roles[0].country_code").value("CH"))
                .andExpect(jsonPath("\$.data.roles[0].business_details.name").value("Smart Charging Network Node"))
    }
    ...
    fun `When POST credentials then return broker credentials and TOKEN_C`() {
        val platform = PlatformEntity(
                ...
                rules = ScnRules(signatures = true, blacklist = false, whitelist = false))
        ...
        mockMvc.perform(post("/scpi/2.2/credentials")
                ...
                .andExpect(jsonPath("\$.data.url").value("http://my.broker.com/scpi/versions"))
                .andExpect(jsonPath("\$.data.roles", Matchers.hasSize<Array<CredentialsRole>>(1)))
                .andExpect(jsonPath("\$.data.roles[0].role").value("HUB"))
                .andExpect(jsonPath("\$.data.roles[0].party_id").value("SCN"))
                .andExpect(jsonPath("\$.data.roles[0].country_code").value("CH"))
    ...
    fun `When PUT credentials then return broker credentials and new TOKEN_C`() {
        val platform = PlatformEntity(
                ...
                rules = ScnRules(signatures = false, whitelist = true, blacklist = false))
        ...
        mockMvc.perform(put("/scpi/2.2/credentials")
                .header("Authorization", "Token ${platform.auth.tokenC}")
                .contentType(MediaType.APPLICATION_JSON)
                .content(jacksonObjectMapper().writeValueAsString(Credentials(
    ...
    @Test
    fun `When DELETE credentials then return SCPI success message`() {
        ...
        every { scnRulesListRepo.deleteByPlatformID(platform.id) } just Runs

        mockMvc.perform(delete("/scpi/2.2/credentials")
                .header("Authorization", "Token ${platform.auth.tokenC}"))
                .andExpect(status().isOk)
                .andExpect(jsonPath("\$.status_code").value(ScpiStatus.SUCCESS.code))
                .andExpect(jsonPath("\$.status_message").doesNotExist())
