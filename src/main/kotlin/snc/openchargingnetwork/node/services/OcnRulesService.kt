...
class OcnRulesService(private val platformRepo: PlatformRepository,
                      private val scnRulesListRepo: OcnRulesListRepository) {
    ...
    private fun getModules(scnRulesListEntity: OcnRulesListEntity): OcnRulesListParty {
    ...
    fun updateWhitelist(authorization: String, parties: List<OcnRulesListParty>) {
        ...
        scnRulesListRepo.saveAll(parties.map { OcnRulesListEntity(
    ...
    fun appendToWhitelist(authorization: String, body: OcnRulesListParty) {
        ...
        if (scnRulesListRepo.existsByCounterparty(BasicRole( id = body.id, country = body.country).toUpperCase())) {
            throw OcpiClientInvalidParametersException("Party already on SCN Rules whitelist")
        }
        ...
        // 7. add to whitelist
        scnRulesListRepo.save(OcnRulesListEntity(
                platformID = platform.id!!,
    ...
    fun appendToBlacklist (authorization: String, body: OcnRulesListParty) {
        ...
        // 5. check entry does not already exist
        if (scnRulesListRepo.existsByCounterparty(BasicRole( id = body.id, country = body.country).toUpperCase())) {
            throw OcpiClientInvalidParametersException("Party already on SCN Rules blacklist")
        }
    ...
    fun deleteFromWhitelist(authorization: String, party: BasicRole) {
        ...
        if (platform.rules.blacklist || !platform.rules.whitelist) {
            throw OcpiClientGenericException("Cannot delete entry from SCN Rules whitelist")
        }
    ...
    fun deleteFromBlacklist(authorization: String, party: BasicRole) {
        ...
        if (platform.rules.whitelist || !platform.rules.blacklist) {
            throw OcpiClientGenericException("Cannot delete entry from SCN Rules blacklist")
        }
    ...
    private fun assertListNotActive(platform: PlatformEntity, type: OcnRulesListType) {
        ...
        if (list) {
            throw OcpiClientGenericException("SCN Rules whitelist and blacklist cannot be active at same time")
        }
    }
