...
import snc.smartchargingnetwork.node.integration.parties.MspServer
...
class IntegrationTest {
    ...
    @Test
    fun registry() {
        val response1 = khttp.get("http://localhost:8080/scn/registry/node/de/msp")
        val response2 = khttp.get("http://localhost:8081/scn/registry/node/de/msp")
    ...
    fun basic_request() {
        for (cpo in cpos) {
            ...
            // check signature
            val notary = Notary.deserialize(json.getString("scn_signature"))
    ...
    fun paginated_request() {
        for (cpo in cpos) {
            ...
            val pattern = "<http://localhost:8080/scpi/sender/2.2/locations/page/\\d+>; rel=\"next\"".toPattern()
    ...
    fun restful_post_request() {
        for (cpo in cpos) {
            ...
            val pattern = "${cpo.server.node}/scpi/receiver/2.2/cdrs/\\d".toPattern()
            ...
            val sig = Notary.deserialize(json.getString("scn_signature"))
