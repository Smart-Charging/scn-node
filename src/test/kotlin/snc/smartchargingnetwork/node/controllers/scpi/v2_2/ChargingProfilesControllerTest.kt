...
class ChargingProfilesControllerTest(@Autowired val mockMvc: MockMvc) {
    ...
    @Test
    fun `When POST sender result return SCPI success response`() {
        ...
        val response = ResponseEntity.status(HttpStatus.ACCEPTED).body(ScpiResponse<Unit>(statusCode = 1000))
        ...
        mockMvc.perform(post("/scpi/2.2/sender/chargingprofiles/result/12345")
                .header("Authorization", "Token token-c")
                .header("X-Request-ID", "666")
                .header("X-Correlation-ID", "666")
                .header("SCPI-From-Country-Code", "CH")
                .header("SCPI-From-Party-ID", "SNC")
                .header("SCPI-To-Country-Code", "DE")
                .header("SCPI-To-Party-ID", "ABC")
    ...
    @Test
    fun `When PUT sender chargingprofiles return SCPI success response`() {
        val request = ScpiRequestVariables(
                module = ModuleID.CHARGING_PROFILES,
                interfaceRole = InterfaceRole.SENDER,
        ...
        mockMvc.perform(put("/scpi/2.2/sender/chargingprofiles/1234567890")
                .header("Authorization", "Token token-c")
                .header("X-Request-ID", "666")
                .header("X-Correlation-ID", "666")
                .header("SCPI-From-Country-Code", "CH")
    ...
    fun `When GET receiver chargingprofiles return ChargingProfileResponse`() {
        ...
        mockMvc.perform(get("/scpi/2.2/receiver/chargingprofiles/0987654321")
                .header("Authorization", "Token token-c")
                .header("X-Request-ID", "666")
                .header("X-Correlation-ID", "666")
                .header("SCPI-From-Country-Code", "CH")
    ...
    fun `When PUT receiver chargingprofiles return ChargingProfileResponse`() {
        ...
        mockMvc.perform(put("/scpi/2.2/receiver/chargingprofiles/0102030405")
                .header("Authorization", "Token token-c")
                .header("X-Request-ID", "666")
                .header("X-Correlation-ID", "666")
                .header("SCPI-From-Country-Code", "CH")
    ...
    fun `When DELETE receiver chargingprofiles return ChargingProfileResponse`() {
        val request = ScpiRequestVariables(
                ...
                urlEncodedParams = mapOf("response_url" to "https://scsp.io/scpi/callback/369"))

        val requestHandler: RequestHandler<ChargingProfileResponse> = mockk()

        every { requestHandlerBuilder.build<ChargingProfileResponse>(request) } returns requestHandler
        every {
            requestHandler.validateSender().forwardModifiableRequest("https://scsp.io/scpi/callback/369", any()).getResponse()
        } returns ResponseEntity
                .status(200)
                .body(ScpiResponse(
                        statusCode = 1000,
        ...
        mockMvc.perform(delete("/scpi/2.2/receiver/chargingprofiles/333666999")
                .header("Authorization", "Token token-c")
                .header("X-Request-ID", "666")
                .header("X-Correlation-ID", "666")
                .header("SCPI-From-Country-Code", "CH")
                ...
                .header("SCPI-To-Country-Code", "DE")
                .header("SCPI-To-Party-ID", "ABC")
                .param("response_url", "https://scsp.io/scpi/callback/369")
                .contentType(MediaType.APPLICATION_JSON)
                .content(jacksonObjectMapper().writeValueAsString(request.body)))
                .andExpect(MockMvcResultMatchers.content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(MockMvcResultMatchers.jsonPath("\$.status_code").value(1000))
