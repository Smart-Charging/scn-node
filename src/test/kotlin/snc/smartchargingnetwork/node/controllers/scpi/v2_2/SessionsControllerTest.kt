...
import snc.smartchargingnetwork.node.data.exampleSession
...
class SessionsControllerTest(@Autowired val mockMvc: MockMvc) {
    ...
    fun `When GET sender Sessions return paginated session list`() {
        ...
        responseHeaders["Link"] = "https://client.scn.co/scpi/sender/2.2/sessions/page/2247; rel=\"next\""
        ...
        every { mockRequestHandler.validateSender().forwardRequest().getResponseWithPaginationHeaders() } returns ResponseEntity
                ...
                .body(ScpiResponse(statusCode = 1000, data = arrayOf(exampleSession)))

        mockMvc.perform(get("/scpi/sender/2.2/sessions")
                ...
                .andExpect(header().string("Link", "https://client.scn.co/scpi/sender/2.2/sessions/page/2247; rel=\"next\""))
    ...
    fun `When GET sender Sessions page should return proxied sessions list page`() {
        ...
        val responseHeaders = HttpHeaders()
        responseHeaders["Link"] = "https://client.scn.co/scpi/sender/2.2/sessions/page/2248; rel=\"next\""
        responseHeaders["X-Limit"] = "20"
        responseHeaders["X-Total-Count"] = "87"
        ...
        mockMvc.perform(get("/scpi/sender/2.2/sessions/page/${requestVariables.urlPathVariables}")
                .header("Authorization", "Token token-c")
                .header("X-Request-ID", requestVariables.headers.requestID)
                .header("X-Correlation-ID", requestVariables.headers.correlationID)
                ...
                .param("limit", "100"))
                .andExpect(status().isOk)
                .andExpect(header().string("Link", "https://client.scn.co/scpi/sender/2.2/sessions/page/2248; rel=\"next\""))
                .andExpect(header().string("X-Limit", "20"))
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
    ...
    fun `When PUT charging_preferences return ChargingPreferencesResponse`() {
        ...
        every { mockRequestHandler.validateSender().forwardRequest().getResponse() } returns ResponseEntity
                .status(200)
                .body(ScpiResponse(statusCode = 1000, data = ChargingPreferencesResponse.NOT_POSSIBLE))

        mockMvc.perform(put("/scpi/sender/2.2/sessions/2247/charging_preferences")
                .header("Authorization", "Token token-c")
                .header("X-Request-ID", requestVariables.headers.requestID)
                .header("X-Correlation-ID", requestVariables.headers.correlationID)
                .header("SCPI-from-country-code", requestVariables.headers.sender.country)
    ...
    fun `When GET receiver Session return session object`() {
        ...
        every { mockRequestHandler.validateSender().forwardRequest().getResponse() } returns ResponseEntity
                .status(200)
                .body(ScpiResponse(statusCode = 1000, data = exampleSession))

        mockMvc.perform(get("/scpi/receiver/2.2/sessions/${sender.country}/${sender.id}/$sessionID")
                .header("Authorization", "Token token-c")
                .header("X-Request-ID", requestVariables.headers.requestID)
                .header("X-Correlation-ID", requestVariables.headers.correlationID)
                .header("SCPI-from-country-code", sender.country)
    ...
    @Test
    fun `When PUT receiver Session return SCPI success`() {
        ...
        mockMvc.perform(put("/scpi/receiver/2.2/sessions/${sender.country}/${sender.id}/$sessionID")
                .header("Authorization", "Token token-c")
                .header("X-Request-ID", requestVariables.headers.requestID)
                .header("X-Correlation-ID", requestVariables.headers.correlationID)
                .header("SCPI-from-country-code", sender.country)
    ...
    @Test
    fun `When PATCH receiver Session return SCPI success`() {
        ...
        mockMvc.perform(patch("/scpi/receiver/2.2/sessions/${sender.country}/${sender.id}/$sessionID")
                .header("Authorization", "Token token-c")
                .header("X-Request-ID", requestVariables.headers.requestID)
                .header("X-Correlation-ID", requestVariables.headers.correlationID)
                .header("SCPI-from-country-code", sender.country)
